...
3. 왼쪽 메뉴 **Authentication > Users** 에서 **Add user** 로 교사 로그인 계정을 만듭니다 (이메일 + 비밀번호). 학년마다 담당 교사가 다르면 여러 명 만드세요.
4. 방금 만든 계정의 **UID**를 사용자 목록에서 복사한 뒤, SQL Editor에서 아래 SQL로 담당 학년을 등록합니다 (이 작업을 안 하면 그 교사는 로그인해도 게시물이 하나도 안 보입니다):
   ```sql
   insert into public.teachers (user_id, grade)
   values ('여기에_UID_붙여넣기', '1')  -- 담당 학년: '1' / '2' / '3' / 전체 관리자는 'ALL'
   on conflict (user_id) do update set grade = excluded.grade;
   ```
5. 왼쪽 메뉴 **Project Settings > API** (또는 **Data API**) 에서 두 값을 복사해둡니다.
   - `Project URL`
   - `anon public` / `Publishable` 키 (공개되어도 안전한 키입니다. 실제 보안은 2번에서 설정한 권한이 담당합니다.)

## 2단계. 설정 파일 수정

[`js/config.js`](js/config.js) 파일을 열어 아래 값을 채우세요.

- `SUPABASE_URL`, `SUPABASE_ANON_KEY` : 1단계 5번에서 복사한 값
- `SCHOOL_LAT`, `SCHOOL_LNG` : 우리 학교 좌표
  - 구글맵에서 학교 건물을 우클릭 → 맨 위에 뜨는 좌표(위도, 경도)를 복사
- `GRADES` : 학년 목록 (기본값 1~3학년)
- `CLASSES` : 반 이름과 핀 색상 (기본값 1~9반, 모든 학년이 공동으로 사용, 필요하면 자유롭게 수정)
...
1. 위쪽 학년 탭에서 본인 학년을 선택
2. 지도에서 발견한 위치를 클릭하거나, "📍 내 위치로 기록" 버튼으로 GPS 현재 위치를 사용
3. 이름/학년/반, 생물 이름, 분류(종·속·과·목·강·문·계), 서식지, 형태, 특징, 습성, 사진을 입력 후 등록
4. 등록한 내용은 **교사가 승인하기 전까지 다른 사람에게는 보이지 않습니다** (제출한 학생 화면에는 "승인 대기중"으로 바로 보임)
5. 반별 색상 체크박스로 원하는 반만 지도에 표시할 수 있음

### 교사 (admin.html)

1. Supabase에서 만든 이메일/비밀번호로 로그인
2. 로그인하면 **본인이 담당하는 학년의 게시물만** 보이고 관리할 수 있음 (다른 학년 데이터는 아예 보이지 않음 — 서버 권한으로 강제됨). 담당 학년이 'ALL'로 등록된 계정은 전체 학년을 봄
3. 대기중인 게시물을 확인 후 **승인** 또는 **삭제**
4. 이미 승인된 게시물도 부적절하면 **삭제** 가능
5. 학기 말 **📦 전체 백업** 버튼 클릭 → (본인 담당 학년의) 전체 데이터(CSV)와 사진이 담긴 zip 파일이 자동으로 다운로드됨

## 무료 요금제 관련 참고사항

- Supabase 무료 요금제: DB 500MB, Storage 1GB, 월 2GB 대역폭 등 한도가 있습니다. 사진은 클라이언트에서 자동 압축(1200px, 70~80% 품질, 보통 개당 수십~수백 KB)되어 올라가므로 700명 규모(3개 학년 × 9개 반)라도 충분히 여유가 있습니다. 학생들이 짧은 시간 동안 동시에 접속해도 각자 가벼운 요청(등록/조회) 몇 번만 오가는 정적 사이트 구조라 무리 없이 처리됩니다.
